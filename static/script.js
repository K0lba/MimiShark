...
tds.forEach(function (item) {
    item.onclick = function () {
        ...
        for (var b = 0; b < (string.length)/47 ; b++) {
            ...
            elem3.innerHTML = pcap_data[i-1]['ascii'].substring(b * 16 + b, (b * 16) + 16 + b);;
            ascii.appendChild(elem3);
        }
        var len = pcap_data[i-1]['decode'].length;
        make_decode(pcap_data[i-1]['decode'].substring(0,len-26).split(','));
    };
});

function make_decode(li_data){
    var temp = [];
    var index = [];
...
function make_pa(text, count){
    ...
    decode_a.classList.add("decode_head", "dropdown-toggle");
